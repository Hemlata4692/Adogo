...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.type == HMSegmentedControlTypeText) {
		...
		[self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
            ...
            CALayer *imageLayer = [CALayer layer];
            imageLayer.frame = CGRectMake(imageRect.origin.x, 10, imageRect.size.width, imageRect.size.height);;
...
//            titleLayer.backgroundColor = [UIColor greenColor].CGColor;
            titleLayer.frame = CGRectMake(textRect.origin.x, imageLayer.frame.origin.y + imageLayer.frame.size.height+5, textRect.size.width, textRect.size.height);
            [self.scrollView.layer addSublayer:titleLayer];
